...
public class TestDb extends AndroidTestCase {
    ...
    */
    public long testLocationTable() {
        ...
        assertTrue(locationRowId != -1); //if returns -1, we did not get a row back
        ...
        //move cursor to check that there is only one record
        assertFalse("Error: more than one record returned from location query", cursor.moveToNext());

        //CLOSE ALL (prevent resource leak)
        cursor.close();
        db.close();
    ...
    public void testWeatherTable() {
        ...
        assertTrue(weatherRowId != -1); //if returns -1, we did not get a row back
